  ...
  return (
    <div className={`${course.bgColor} rounded-2xl p-5 shadow-lg hover:shadow-xl transition-all duration-300 border ${course.borderColor} transform hover:-translate-y-1 h-full flex flex-col group relative overflow-hidden`}>
      ...
      <div className="flex-1 space-y-4 relative z-10">
        {course.concepts && (
          <div className="bg-white/70 backdrop-blur-sm rounded-xl p-3 border border-white/50">
            ...
            <div className="space-y-1">
              {course.concepts.slice(0, isExpanded ? course.concepts.length : 2).map((c, idx) => (
              ...
              {course.concepts.length > 2 && (
                <button 
                  onClick={() => setIsExpanded(!isExpanded)}
                  className={`text-xs font-medium ${course.textColor} mt-1 flex items-center gap-1`}
                >
                  {isExpanded ? 'Show less' : `+${course.concepts.length - 2} more`}
        ...
        {(course.activity || course.activities) && isExpanded && (
          <div className="bg-white/50 backdrop-blur-sm rounded-xl p-3 border border-white/40">
            <h4 className={`text-xs font-bold ${course.textColor} mb-2 flex items-center gap-1`}>
...
const CoursesSection = ({ section }) => {
  ...
  // Always show all courses on desktop, show 1 on mobile initially
  const coursesToShow = isMobile ? (showAll ? section.courses : [section.courses[0]]) : section.courses;
  const hasMore = isMobile && section.courses.length > 1;
        ...
        {hasMore && (
          <div className="mt-6 text-center">
            <button
              ...
              className={`inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r ${section.gradient} text-white font-bold text-sm shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1`}
            >
              {showAll ? 'Show Less' : `+${section.courses.length - 1} More Courses`}
              {showAll ? <ChevronUp size={16} /> : <Plus size={16} />}
            </button>
          </div>
        )}
      </div>
